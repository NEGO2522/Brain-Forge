                      ...
                      <div className="max-h-64 overflow-y-auto p-2 grid grid-cols-1 md:grid-cols-2 gap-1">
                        {TECH_DOMAINS.filter(o => o.toLowerCase().includes(searchTerm.toLowerCase())).map(option => (
                          <div 
                            ...
                            className={`px-4 py-3 text-[10px] uppercase font-bold tracking-widest rounded-xl cursor-pointer transition-all ${formData.techStack.includes(option) ? 'bg-amber-500 text-black' : 'hover:bg-white/5 text-gray-500 hover:text-white'}`}
                          >
                            {option}
                          </div>
                        ))}
                ...
                <AnimatePresence>
                  {showCustomInput && (
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} className="mt-4 flex gap-2">
                      ...
                      />
                      <button type="button" onClick={handleAddCustomTech} className="bg-amber-500 text-black px-6 rounded-xl font-black text-[10px] uppercase hover:bg-white transition-all flex items-center gap-2"><FiPlus /> Add</button>
