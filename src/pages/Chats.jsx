...
const Chats = () => {
  ...
  useEffect(() => {
    ...
    const roomId = getRoomId(currentUser.uid, selected.userId);
    const q = query(
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (selected.userId !== currentUser.uid) {
        const notifRef = doc(
          collection(db, 'notifications', selected.userId, 'items')
        );
  ...
  const handleQuickAction = async (type) => {
    ...
    const skill = selected.techStack?.split(',')[0]?.trim() || 'your field';
    const messages = {
      doubt:    `Hi ${firstName}, I have a doubt related to ${skill} and would really appreciate your help in clearing it.`,
      guidance: `Hi ${firstName}, I'd love to get your guidance on how to grow in ${skill}. Could you share some direction or advice?`,
      resume:   `Hi ${firstName}, would you be open to doing a quick resume review for me? I'd really value your feedback and suggestions.`,
      project:  `Hi ${firstName}, I'm working on a project and could use some expert help. Would you be open to guiding me through it?`,
    };
    const text = messages[type];
    if (!text) return;
    setSending(true);
    const roomId = getRoomId(currentUser.uid, selected.userId);
    const senderName = currentUser.displayName || currentUser.email?.split('@')[0] || 'You';
    try {
      await addDoc(collection(db, 'chats', roomId, 'messages'), {
        text, senderId: currentUser.uid, senderName, createdAt: serverTimestamp(),
      });
      if (selected.userId !== currentUser.uid) {
        const notifRef = doc(collection(db, 'notifications', selected.userId, 'items'));
        await setDoc(notifRef, {
          type: 'message', senderId: currentUser.uid, senderName,
          senderPhoto: currentUser.photoURL || null,
          text: text.length > 60 ? text.slice(0, 60) + '…' : text,
          chatRoomId: roomId, read: false, createdAt: serverTimestamp(),
        });
      }
    } catch (err) {
      ...
      >
        ...
        <div className="px-4 py-3 border-b border-white/[0.06] flex-shrink-0">
          <div className="relative">
            <FiSearch
              className="absolute left-3 top-1/2 -translate-y-1/2 text-amber-500/40"
              size={14}
            />
            <input
              type="text"
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {profile.techStack && (
                    <p className="text-[9px] text-amber-500/50 font-black uppercase tracking-wider truncate mt-0.5">
                      {profile.techStack.split(',').slice(0, 2).join(' · ')}
                    </p>
                  )}
                </div>
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <p className="text-[10px] text-gray-500 truncate uppercase tracking-widest">
                    {selected.techStack?.split(',').slice(0, 2).join(' · ') || 'No stack listed'}
                  </p>
                </div>
              </div>
          ...
          <AnimatePresence initial={false}>
            {messages.map((msg) => {
              ...
              return (
                ...
                >
                  <div
                    className={`max-w-[75%] md:max-w-[65%] flex flex-col gap-1 ${
            ...
            <div className="relative group max-w-4xl mx-auto">
              ...
              <div className="relative flex items-end gap-2 bg-[#121212] border border-white/10 rounded-[2rem] p-2 pl-5 focus-within:border-amber-500/40 transition-all">
                ...
                <motion.button
                  ...
                  className="h-11 w-11 flex items-center justify-center rounded-full bg-amber-500 hover:bg-amber-400 text-black shadow-lg shadow-amber-500/20 disabled:opacity-20 transition-all flex-shrink-0 mb-1"
                >
                  {sending ? (
