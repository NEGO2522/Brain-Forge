...
const Chat = () => {
  ...
  useEffect(() => {
    const u = onAuthStateChanged(auth, setCurrentUser);
    return u;
  }, []);

  const chatRoomId = currentUser && profile?.userId
    ? [currentUser.uid, profile.userId].sort().join('_') : null;
  ...
  const doSend = async (text) => {
    ...
    const sName = currentUser.displayName || currentUser.email?.split('@')[0] || 'You';
    try {
      await addDoc(collection(db, 'chats', chatRoomId, 'messages'), {
        text: text.trim(), senderId: currentUser.uid, senderName: sName, createdAt: serverTimestamp(),
      });
      const rid = profile?.userId;
      if (rid && rid !== currentUser.uid) {
        await setDoc(doc(collection(db, 'notifications', rid, 'items')), {
          type: 'message', senderId: currentUser.uid, senderName: sName,
          senderPhoto: currentUser.photoURL || null,
          text: text.length > 60 ? text.slice(0, 60) + '…' : text,
          chatRoomId, read: false, createdAt: serverTimestamp(),
        });
      }
  ...
  const quickSend = (type) => {
    ...
    const s = profile?.techStack?.split(',')[0]?.trim() || 'your field';
    const map = {
      doubt:    `Hi ${n}, I have a doubt related to ${s} and would really appreciate your help in clearing it.`,
      guidance: `Hi ${n}, I'd love to get your guidance on how to grow in ${s}. Could you share some direction or advice?`,
      resume:   `Hi ${n}, would you be open to doing a quick resume review for me? I'd really value your feedback and suggestions.`,
      project:  `Hi ${n}, I'm working on a project and could use some expert help. Would you be open to guiding me through it?`,
    };
    doSend(map[type]);
  };
  ...
  const techTags = profile?.techStack?.split(',').map(t => t.trim()).filter(Boolean) || [];
  ...
  return (
    <div className="h-screen w-full flex flex-col overflow-hidden bg-[#030303] text-white pt-16 md:pt-20 relative">
      ...
      <header className="relative z-20 flex-shrink-0 flex items-center gap-4 px-4 md:px-8 py-3 bg-black/60 backdrop-blur-2xl border-b border-white/[0.06]">
        ...
        <div className="flex items-center gap-3 flex-1 min-w-0">
          <div className="relative flex-shrink-0">
            <div className="w-11 h-11 rounded-2xl bg-gradient-to-br from-amber-500/30 to-amber-600/10 border border-amber-500/30 flex items-center justify-center text-amber-400 shadow-[0_0_20px_rgba(245,158,11,0.15)]">
              <FiUser size={20} />
            </div>
            {/* online dot */}
            <span className="absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full bg-emerald-500 border-2 border-black shadow-[0_0_8px_rgba(16,185,129,0.8)]" />
          </div>
            ...
            <div className="flex items-center gap-3 mt-0.5">
              ...
              {techTags.length > 0 && (
                <div className="hidden md:flex items-center gap-1">
                  <FaCpu size={9} className="text-amber-500/50" />
                  <span className="text-[10px] text-amber-500/60 font-black uppercase tracking-wide truncate">
                    {techTags.slice(0, 2).join(' · ')}
                  </span>
                </div>
              )}
            </div>
      ...
      ══════════════════════════════════════════ */}
      <main
        className="relative z-10 flex-1 overflow-y-auto py-6 px-4 md:px-[12%] space-y-3"
      ...
      >
        ...
        {messages.length === 0 && (
          ...
          >
            {/* profile card in empty state */}
            <div className="relative w-20 h-20 rounded-3xl bg-gradient-to-br from-amber-500/20 to-amber-600/5 border border-amber-500/20 flex items-center justify-center shadow-[0_0_40px_rgba(245,158,11,0.1)]">
              <FiUser size={36} className="text-amber-500/60" />
              <span className="absolute -bottom-1 -right-1 w-5 h-5 rounded-full bg-emerald-500 border-2 border-[#030303] shadow-[0_0_10px_rgba(16,185,129,0.6)]" />
            </div>
            ...
            {techTags.length > 0 && (
              <div className="flex flex-wrap justify-center gap-2 max-w-xs">
                {techTags.map(t => (
                  <span key={t} className="px-3 py-1.5 bg-white/5 border border-white/10 rounded-full text-[9px] font-black uppercase tracking-widest text-gray-400">
                    {t}
                  </span>
                ))}
              </div>
        ...
        <AnimatePresence initial={false}>
          {messages.map((msg, i) => {
            ...
            const showSender = !isMe && msg.senderName !== prevMsg?.senderName;
            return (
              <motion.div
                key={msg.id}
              ...
              >
                ...
                <div className={`flex flex-col gap-1 max-w-[78%] md:max-w-[62%] ${isMe ? 'items-end' : 'items-start'}`}>
                  {showSender && (
                    <span className="text-[9px] text-amber-500/50 font-black uppercase tracking-widest px-1">
                      {msg.senderName}
                    </span>
                  )}
                  ...
                  <div className={`relative group px-4 py-3 rounded-2xl text-sm leading-relaxed break-words ${
                    isMe
                      ...
                      : 'bg-white/[0.05] border border-white/[0.08] text-gray-100 rounded-bl-sm backdrop-blur-md shadow-[0_4px_20px_rgba(0,0,0,0.3)]'
                  }`}>
                    {msg.text}
                    {/* subtle shine on my messages */}
                    {isMe && (
                      <div className="absolute inset-0 rounded-2xl rounded-br-sm bg-gradient-to-br from-white/20 to-transparent pointer-events-none" />
                    )}
                  </div>
        ...
        {currentUser ? (
          ...
          <div className="text-center py-4">
            <button
              ...
              className="px-8 py-3 bg-amber-500 text-black font-black uppercase tracking-widest text-xs rounded-full hover:bg-amber-400 transition-all shadow-[0_0_20px_rgba(245,158,11,0.3)]"
            >
              Sign In to Chat
            </button>
          </div>
