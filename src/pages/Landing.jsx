...
import { FaHandsHelping, FaGlobe, FaUserGraduate, FaGithub, FaLinkedin, FaArrowRight, FaStar } from 'react-icons/fa';
import { FiArrowRight, FiZap, FiMessageSquare, FiSearch, FiStar, FiCode, FiShield, FiBriefcase } from 'react-icons/fi';
...
const ParticleCloud = () => {
  ...
  useFrame((s) => {
    if (ref.current) {
      ref.current.rotation.y = s.clock.getElapsedTime() * 0.02;
      ref.current.rotation.x = s.clock.getElapsedTime() * 0.01;
    }
  });
  return (
    <points ref={ref}>
      ...
      <PointMaterial size={0.04} color="#ffffff" transparent opacity={0.25} sizeAttenuation depthWrite={false} />
    </points>
  );
};
...
const HeroScene = ({ isMobile }) => (
  <Canvas
    camera={{ position: [0, 0, isMobile ? 9 : 7], fov: 50 }}
    gl={{ antialias: true, alpha: true, powerPreference: 'high-performance' }}
    style={{ background: 'transparent' }}
    onCreated={({ gl }) => { gl.setClearColor(0x000000, 0); }}
  >
    <Suspense fallback={null}>
      <Stars radius={60} depth={40} count={1000} factor={3} saturation={0} fade speed={0.5} />
      <ParticleCloud />
      <ConnectionLines />
    ...
    <pointLight position={[4, 3, 4]} intensity={35} color="#67e8f9" />
    <pointLight position={[-4, -2, 3]} intensity={18} color="#a78bfa" />
    <pointLight position={[0, 4, 2]} intensity={12} color="#f59e0b" />
  </Canvas>
);

/* ══════════════════════════════════════════
   MENTOR CARD STACK (Hero Right Side)
══════════════════════════════════════════ */
const MENTOR_CARDS = [
  {
    name: 'Rahul Sharma',
    ...
    rating: 4.9,
    sessions: 128,
    tags: ['Ethical Hacking', 'CTF', 'Networking'],
    price: '₹149/hr',
    initials: 'RS',
  },
  {
...
const MentorCardStack = () => {
  const [activeIdx, setActiveIdx] = useState(0);

  // Auto-cycle cards every 3 seconds
  useEffect(() => {
    const t = setInterval(() => setActiveIdx(i => (i + 1) % MENTOR_CARDS.length), 3000);
    return () => clearInterval(t);
  }, []);

  return (
    <div className="hidden lg:flex flex-col items-end justify-center gap-5 w-full">
      ...
      >
        ...
        <span className="text-[10px] font-mono uppercase tracking-[0.35em] text-zinc-400">
          Active Senior Mentors
        </span>
      </motion.div>

      {/* Cards */}
      <div className="relative w-full max-w-[340px] self-end">
        {MENTOR_CARDS.map((mentor, i) => {
          const offset = (i - activeIdx + MENTOR_CARDS.length) % MENTOR_CARDS.length;
          ...
          return (
            <motion.div
              ...
              animate={{
                y:       offset === 0 ? 0 : offset === 1 ? 12 : 22,
                scale:   offset === 0 ? 1 : offset === 1 ? 0.95 : 0.90,
              ...
              style={{ position: offset === 0 ? 'relative' : 'absolute' }}
            >
              <div
                className="w-full rounded-2xl p-5 border backdrop-blur-2xl transition-all duration-300"
                style={{
                  backgroundColor: isActive ? 'rgba(20,20,30,0.92)' : 'rgba(15,15,22,0.80)',
                  borderColor: isActive ? `${mentor.color}44` : 'rgba(255,255,255,0.06)',
                  boxShadow: isActive
                    ? `0 20px 60px rgba(0,0,0,0.6), 0 0 0 1px ${mentor.color}22`
              ...
              >
                ...
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3">
                    {/* Avatar */}
                    <div
                      className="w-11 h-11 rounded-xl flex items-center justify-center text-sm font-black text-black flex-shrink-0"
                    ...
                    <div>
                      ...
                      <p className="text-[10px] font-mono" style={{ color: mentor.color }}>{mentor.college}</p>
                    </div>
                  </div>
                  ...
                  {isActive && (
                    <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[9px] font-black uppercase tracking-widest"
                      style={{ backgroundColor: 'rgba(52,211,153,0.12)', border: '1px solid rgba(52,211,153,0.25)', color: '#34d399' }}>
                ...
                <div className="flex items-center gap-2 mb-3">
                  <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[10px] font-bold"
                    style={{ backgroundColor: `${mentor.color}14`, color: mentor.color }}>
                    {mentor.icon} {mentor.domain}
                  </div>
                  <span className="text-[9px] text-zinc-500 font-mono uppercase tracking-wider">{mentor.year}</span>
                </div>
                ...
                <div className="flex flex-wrap gap-1.5 mb-4">
                  {mentor.tags.map(tag => (
                    <span key={tag}
                      className="px-2 py-0.5 rounded-md text-[8px] font-bold uppercase tracking-wide text-zinc-400"
                ...
                <div className="flex items-center justify-between pt-3"
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-[10px] text-zinc-500">{mentor.sessions} sessions</span>
                  </div>
                  {/* Price */}
                  <span className="text-sm font-black" style={{ color: mentor.color }}>{mentor.price}</span>
                </div>
              </div>
            </motion.div>
          );
        ...
        {/* Spacer to keep the stack height correct */}
        <div style={{ height: 22 }} />
      </div>
        ...
        {MENTOR_CARDS.map((_, i) => (
          <button
            ...
            style={{
              ...
              height: 6,
              backgroundColor: activeIdx === i ? ACCENT : 'rgba(255,255,255,0.20)',
            }}
          />
        ))}
      </motion.div>
      ...
      <motion.div
        ...
        className="flex items-center justify-between w-full max-w-[340px] self-end px-4 py-3 rounded-xl"
        style={{ backgroundColor: 'rgba(245,158,11,0.07)', border: '1px solid rgba(245,158,11,0.18)' }}
      >
        {/* Avatar pile */}
        <div className="flex -space-x-2">
          {['#f59e0b', '#67e8f9', '#a78bfa', '#4ade80', '#f472b6'].map((c, i) => (
            <div key={i}
              className="w-7 h-7 rounded-full border-2 border-black flex items-center justify-center text-[9px] font-black text-black flex-shrink-0"
        ...
        <div className="text-right">
          <p className="text-xs font-black text-white">500+ Mentors</p>
          <p className="text-[9px] text-zinc-500 uppercase tracking-wider">across India</p>
        </div>
      </motion.div>
...
const StatItem = ({ value, label }) => (
  <div className="flex flex-col items-center lg:items-start">
    <span className="text-4xl font-black font-serif text-amber-500">{value}</span>
    <span className="text-[10px] uppercase tracking-[0.3em] font-bold mt-1 text-zinc-500">{label}</span>
  </div>
);
...
const Step = ({ num, title, desc, last }) => {
  ...
  return (
    <motion.div ref={ref} initial={{ opacity: 0, x: -20 }} animate={inView ? { opacity: 1, x: 0 } : {}}
      transition={{ delay: num * 0.12, duration: 0.5 }} className="flex gap-5">
      <div className="flex flex-col items-center">
        <div className="w-10 h-10 rounded-full flex items-center justify-center text-sm font-black flex-shrink-0"
          style={{ backgroundColor: 'rgba(245,158,11,0.15)', border: '1.5px solid rgba(245,158,11,0.50)', color: '#f59e0b' }}>
          {num}
        </div>
        {!last && <div className="w-px flex-1 mt-2" style={{ backgroundColor: 'rgba(255,255,255,0.08)' }} />}
      </div>
      <div className="pb-10">
...
const IsometricStack = () => {
  const pillars = [
    { id: 1, title: 'Discover', icon: <FaGlobe />, tagline: 'Find Your Mentor', details: 'Browse verified seniors from your college by domain, year, and expertise.' },
    { id: 2, title: 'Connect',  icon: <FaHandsHelping />, tagline: 'Reach Out Directly', details: 'Chat directly, get LinkedIn & GitHub, start the conversation.' },
    { id: 3, title: 'Grow',     icon: <FaUserGraduate />, tagline: 'Learn & Earn', details: 'Juniors get affordable guidance. Seniors earn from consultations.' },
  ];
        ...
        {stack.map((item, i) => (
          <motion.div key={item.id} layout
            ...
            <div className="transform rotate-45 flex flex-col items-center gap-3">
              <div className="text-5xl md:text-6xl" style={{ color: i === 0 ? '#fff' : ACCENT }}>{item.icon}</div>
  ...
  return (
    <motion.div ref={ref}
      ...
      <div className="flex justify-center gap-5 text-zinc-400">
        <a href={`https://github.com/${edu.github}`} target="_blank" rel="noreferrer" className="hover:text-amber-400 transition-colors"><FaGithub size={18} /></a>
        <a href={`https://linkedin.com/${edu.linkedin}`} target="_blank" rel="noreferrer" className="hover:text-amber-400 transition-colors"><FaLinkedin size={18} /></a>
      </div>
    </motion.div>
  );
};
  ...
  return (
    <div className="relative w-full min-h-screen overflow-x-hidden bg-black text-white selection:bg-amber-500 selection:text-black">
      ...
      <section className="relative min-h-[100dvh] lg:min-h-screen flex items-center justify-center overflow-hidden pt-20 lg:pt-0">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-12 w-full grid lg:grid-cols-12 gap-12 lg:gap-16 items-center">
        ...
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-16">
            ...
            <h2 className="text-5xl md:text-6xl font-serif tracking-tight text-white">
              Everything you need to<br /><span className="text-amber-500 italic">connect & grow</span>
            </h2>
          </div>
          ...
          <div className="flex flex-col lg:flex-row gap-20 items-start">
            <div className="lg:w-5/12">
              ...
              <h2 className="text-5xl md:text-6xl font-serif tracking-tight text-white mb-12">
                Connect with mentors<br /><span className="text-amber-500 italic">in 4 simple steps</span>
              </h2>
              <div className="space-y-2">
                {steps.map((s, i) => <Step key={i} {...s} last={i === steps.length - 1} />)}
              </div>
            </div>
            <div className="lg:w-7/12"><IsometricStack /></div>
          </div>
        </div>
      </section>
          ...
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
            ...
            <div className="relative z-10">
              ...
              <h2 className="text-5xl md:text-6xl font-serif tracking-tight mb-8 text-white">
                Ready to find your<br /><span className="text-amber-500 italic">college mentor?</span>
              </h2>
