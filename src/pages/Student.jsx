...
import {
  ...
  FiMessageSquare, FiCalendar, FiCode, FiUser, FiArrowRight, FiStar,
  FiUsers, FiGrid, FiBookOpen, FiChevronDown
} from 'react-icons/fi';
...
/* 
   STUDENT CARD
 */
const StudentCard = ({ student, index, currentUserId }) => {
  const navigate  = useNavigate();
  const techStack = student.tech_stack ? student.tech_stack.split(',').map(t => t.trim()).filter(Boolean) : [];

  const linkedinUrl = student.linkedin?.startsWith('http')
    ? student.linkedin : `https://linkedin.com/in/${student.linkedin}`;
  const githubUrl = student.github?.startsWith('http')
  ...
  const handleChat = () => {
    if (currentUserId) navigate(`/chat/${student.id}`, { state: { student } });
    else navigate('/login');
  };
      ...
      <div className="border-b border-black/8 px-5 py-4 flex items-center justify-between gap-2">
        <div className="flex items-center gap-3 min-w-0">
          <div className="w-10 h-10 bg-black flex items-center justify-center flex-shrink-0">
            <span className="text-white text-xs font-black">{initials(student.name)}</span>
          </div>
        ...
        {student.year && (
          <div className="flex items-center gap-1 text-[9px] font-black uppercase tracking-widest text-black/40 border border-black/15 px-2 py-1 flex-shrink-0 whitespace-nowrap">
            <FiCalendar size={8} /> Yr {student.year}
          </div>
        )}
      </div>
      ...
      <div className="flex flex-col flex-1 px-5 py-4 gap-4">
        ...
        <div className="flex flex-col gap-2 flex-1">
          ...
          <div className="flex flex-wrap gap-1.5 content-start">
            ...
            {techStack.length > 4 && (
              <span className="px-2 py-1 text-[9px] font-black uppercase tracking-wide border border-black/15 text-black/35">
                +{techStack.length - 4}
              </span>
            )}
          </div>
              ...
              <div className="flex gap-1">
                {[1,2,3,4,5].map(s => (
                  <button key={s} onClick={() => handleRate(s)}
                    ...
                    <FaStar size={13} color={s <= (hovered || userRating) ? '#000000' : '#e5e5e5'} />
                  </button>
                ))}
              </div>
            </div>
...
const Student = () => {
  useSEO({
    ...
    description: 'Explore the Linkaura student directory. Find senior students to connect with and learn from.',
    keywords: 'student directory, user profiles, tech stack, students network, linkaura',
  });
  ...
  useEffect(() => {
    const init = async () => {
      ...
      // Fetch logged-in student's own profile
      if (userId) {
  ...
  const filtered = students.filter(s =>
    (s.name?.toLowerCase().includes(searchQuery.toLowerCase()) ||
     s.tech_stack?.toLowerCase().includes(searchQuery.toLowerCase())) &&
    (domainFilter === '' || s.domain?.toLowerCase() === domainFilter.toLowerCase()) &&
    s.user_id !== currentUserId &&
    s.year !== '1'
  );

  const techStack = myProfile?.tech_stack
  ...
  return (
    <div className="min-h-screen bg-white text-black" style={{ fontFamily: "'Georgia', serif" }}>
      ...
      <section className="relative border-b border-black/10 overflow-hidden pt-24">
                {/* My Profile strip — only for logged-in users */}
        {!profileLoading && myProfile && (
          <div className="relative z-10 px-6 md:px-12 pt-8 pb-0 max-w-5xl mx-auto" style={{ fontFamily: 'sans-serif' }}>
                ...
                <div className="min-w-0">
                  ...
                  {myProfile.address && (
                    <p className="flex items-center gap-1 text-[9px] uppercase tracking-widest font-black text-black/35 mt-0.5">
                      <FiMapPin size={8} /> {myProfile.address}
                    </p>
                  )}
                </div>
              </div>
              ...
              <div className="flex-1 grid grid-cols-2 sm:grid-cols-4 divide-x divide-y sm:divide-y-0 divide-black/10">
                <div className="px-6 py-5 flex flex-col justify-center">
                  ...
                  <p className="text-sm font-black text-black">
                    {myProfile.year ? `Year ${myProfile.year}` : '—'}
                  </p>
                </div>
                <div className="px-6 py-5 flex flex-col justify-center">
                  <p className="text-[9px] uppercase tracking-[0.3em] font-black text-black/30 mb-1">Tech</p>
                  <p className="text-sm font-black text-black truncate">
                    {techStack.length > 0 ? techStack[0] : '—'}
                    {techStack.length > 1 && <span className="text-black/40"> +{techStack.length - 1}</span>}
                  </p>
                </div>
                <div className="px-6 py-5 flex flex-col justify-center">
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-[10px] font-black text-black/50">
                      {myRatingCount > 0 ? myAvgRating.toFixed(1) : 'No ratings'}
                    </span>
                  </div>
                </div>
                <div className="px-6 py-5 flex flex-col justify-center gap-2">
                  {myProfile.github && (
                    <a
                      href={myProfile.github.startsWith('http') ? myProfile.github : `https://github.com/${myProfile.github}`}
                  ...
                  {myProfile.linkedin && (
                    <a
                      href={myProfile.linkedin.startsWith('http') ? myProfile.linkedin : `https://linkedin.com/in/${myProfile.linkedin}`}
                      target="_blank" rel="noreferrer"
                      className="flex items-center gap-1.5 text-[9px] uppercase tracking-widest font-black text-black/40 hover:text-black transition-colors">
            ...
            {/* TABS */}
            <div className="flex border-b border-black/10 mt-8" style={{ fontFamily: 'sans-serif' }}>
              {tabs.map(tab => (
                <button
                  key={tab.id}
        ...
        <div className="relative z-10 px-6 md:px-12 py-16 max-w-5xl mx-auto">
          {/* Only show hero text when not logged in or on directory tab */}
          {(!myProfile || activeTab === 'directory') && (
            <>
                ...
                <>
                  <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
                    <span className="inline-flex items-center gap-2 px-4 py-1.5 border border-black/15 text-[10px] font-black uppercase tracking-[0.4em] text-black/50 mb-8" style={{ fontFamily: 'sans-serif' }}>
                      <FiUser size={10} /> Senior Students
                    </span>
                  </motion.div>
                  <motion.h1
                  ...
                  <motion.p
                    ...
                    Browse real profiles of senior students in your domain. Connect directly, learn from experience.
                  </motion.p>
                </>
              )}
              ...
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}
                className="relative max-w-4xl" style={{ fontFamily: 'sans-serif' }}>
                <div className="flex gap-4">
                  ...
                  <div className="relative">
                    <select
                      ...
                      className="py-4 pl-4 pr-10 border border-black/15 bg-white outline-none text-sm text-black font-medium focus:border-black transition-colors duration-200 appearance-none cursor-pointer"
                    >
                    ...
                    <FiChevronDown className="absolute right-3 top-1/2 -translate-y-1/2 text-black/30 pointer-events-none" size={14} />
                  </div>
                </div>
              </motion.div>
          ...
          {activeTab === 'directory' && (
            loading ? (
              <div className="flex flex-col items-start gap-3 py-16" style={{ fontFamily: 'sans-serif' }}>
                ...
                <p className="text-[10px] uppercase tracking-widest font-black text-black/40">Loading students...</p>
              </div>
            ) : (
                ...
                {filtered.length > 0 ? (
                  <motion.div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mt-8" layout>
                    {filtered.map((student, i) => (
                      <StudentCard key={student.id} student={student} index={i} currentUserId={currentUserId} />
                    ))}
                  </motion.div>
                ) : (
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                    ...
                    <p className="font-serif italic text-2xl text-black/40">No students found.</p>
          ...
          {activeTab === 'my-profile' && myProfile && (
            <motion.div
              ...
              className="space-y-px border border-black/10 mt-0" style={{ fontFamily: 'sans-serif' }}
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-px bg-black/10">
                {[
                  ...
                  <div key={label} className="bg-white px-7 py-5">
                    ...
                    <p className="text-sm font-black text-black">{value || <span className="text-black/25 font-normal italic">Not set</span>}</p>
                  </div>
                ))}
              </div>
